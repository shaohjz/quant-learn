...
def calc_trade_plan(code: str, entry_price: float, triggered_rule: dict, all_rules: list) -> str:
    ...
    止盈1 = entry_price × 1.15(+15%)
    止盈2 = entry_price × 1.25(+25%)
    盈亏比 = (止盈1 - entry_price) / (entry_price - 止损)
    ...
    plan = (
        ...
        f"├ 止盈1:¥{take_profit_1:.2f}(+15%)\n"
        f"├ 止盈2:¥{take_profit_2:.2f}(+25%)\n"
        f"├ 盈亏比:{rr_label}\n"
    )
...
def process_armed_signals(now: datetime):
    """REQ-063 修复：处理 threshold_state 中 status='armed' 的卖出信号。
    
    armed = close_confirmed_at 已设置，等待次日执行。
    返回 (executed_count, messages_list)。
    """
    try:
        from pathlib import Path
        ...
        armed_flag_dir = OUTPUT_DIR / '.armed_flags'
        armed_flag_dir.mkdir(exist_ok=True)
        ...
        try:
            armed_rows = conn.execute("""
                SELECT id, stock_code, stock_name, rule_name, rule_threshold,
                       close_confirmed_at, close_price, next_day_confirmed_at
                FROM threshold_state
                WHERE status = 'armed'
                  AND close_confirmed_at IS NOT NULL
                ORDER BY close_confirmed_at, id
            """).fetchall()
            
            if not armed_rows:
                logger.info("process_armed_signals: 无 armed 信号")
                return 0, []
            
            logger.info(f"process_armed_signals: 发现 {len(armed_rows)} 个 armed 信号")
            executed_count = 0
            messages = []
            ...
            for row in armed_rows:
                ...
                close_confirmed_at = row['close_confirmed_at']
                close_price = row['close_price'] or 0
                
                # 去重：如果 next_day_confirmed_at 已设置，说明已经执行过
                if row['next_day_confirmed_at']:
                    logger.info(f"  跳过 [{stock_code}] {rule_name}: next_day_confirmed_at 已设置")
                    continue
                
                # close_confirmed_at < today 才执行（次日已到）
                if close_confirmed_at >= today_str:
                    logger.info(f"  等待次日: [{stock_code}] {rule_name} close_confirmed_at={close_confirmed_at}")
                    continue
                
                flag_file = armed_flag_dir / f"{row['id']}_{today_str}.flag"
                if flag_file.exists():
                    logger.info(f"  去重跳过 [{stock_code}] {rule_name}: 今日已处理")
                    continue
                
                # 获取实时价格
                try:
                    from sim.realtime_price import get_latest_prices
                    prices = get_latest_prices([stock_code])
                    cur_price = float(prices.get(stock_code, {}).get('price', 0) or 0)
                except Exception:
                    cur_price = float(close_price or 0)
                if cur_price <= 0:
                    cur_price = float(close_price or 0)
                ...
                level = level_map.get(rule_name, rule_name)
                
                rule = {
                    'code': stock_code,
                    'name': stock_name,
                    'level': level,
                    'trigger': rule_threshold,
                    'dir': 'above' if 'profit' in rule_name else 'below',
                    'message': f'{rule_name} 触发（armed → 执行）',
                    'source': 'threshold',
                }
                ...
                if result.get('trade') is not None:
                    conn.execute("""
                        ...
                        WHERE id = ?
                    """, (today_str, f"armed 执行: {result.get('message', '')}", row['id']))
                    conn.commit()
                    executed_count += 1
                    flag_file.write_text(f"executed at {now.isoformat()}\n")
                    ...
                    if '无持仓' in reason or '无持仓可卖' in reason:
                        conn.execute("""
                            UPDATE threshold_state
                            SET status = 'executed',
                                next_day_confirmed_at = ?,
                                ...
                                updated_at = CURRENT_TIMESTAMP
                            WHERE id = ?
                        """, (today_str, f"armed 已无持仓: {reason}", row['id']))
                        conn.commit()
                        flag_file.write_text(f"skipped(no position) at {now.isoformat()}\n")
                        messages.append(f"ℹ️ armed 信号 [{stock_code}] {rule_name} 已无持仓（可能已卖出）")
            
            return executed_count, messages
        finally:
            conn.close()
    except Exception as e:
        logger.error(f"process_armed_signals 异常: {e}\n{traceback.format_exc()}")
        return 0, []
...
def main():
    ...
    # 必须在交易时段内才能获取实时价格并执行下单
    if not in_trade_hours(now):
        # 非交易时段：只检查是否有 armed 信号需要处理（用缓存价格）
        try:
            triggered_msgs = []
            armed_count, armed_messages = process_armed_signals(now)
            if armed_count > 0:
                logger.info(f"process_armed_signals: 非交易时段执行了 {armed_count} 个 armed 信号")
        except Exception as e:
            logger.warning(f"process_armed_signals 异常: {e}")
    ...
    try:
        armed_count, armed_messages = process_armed_signals(now)
        if armed_count > 0:
            logger.info(f"process_armed_signals: 执行了 {armed_count} 个 armed 信号")
            triggered_msgs.extend(armed_messages)
    except Exception as e:
        logger.warning(f"process_armed_signals 异常: {e}")
    ...
    # REQ-026: 浮亏风控建议生成 - 每次盘中运行都评估浮亏状态
    try:
        from sim.float_loss_advisor import check_critical_float_loss, format_quick_float_loss_alert
        critical_float_loss = check_critical_float_loss(account_id=1)
        if critical_float_loss:
            float_loss_alert = format_quick_float_loss_alert(critical_float_loss)
            if float_loss_alert:
                triggered_msgs.append(float_loss_alert)
                logger.warning(f"REQ-026 浮亏风控: 发现 {len(critical_float_loss)} 只重症浮亏个股")
                # 标记状态避免重复推送
                for pos in critical_float_loss:
                    fid = f"{pos.code}_float_loss_{pos.severity}"
                    if not today_state.get(fid):
                        today_state[fid] = {
                            'triggered_at': now.strftime('%H:%M'),
                            'price': pos.current_price,
                            'trigger': f'float_loss_{pos.severity}',
                            'message': pos.advice,
                        }
    except Exception as e:
        logger.warning(f"REQ-026 浮亏风控检查异常: {e}")
