...
def generate_enhanced_report():
    ...
    cursor.execute("""
        SELECT trade_date, stock_code, stock_name, direction, 
               price, quantity, amount, signal_reason, trade_time,
               signal_detail
        FROM sim_trades
        WHERE trade_date = ?
        ORDER BY trade_time
    ...
    if not trades:
        ...
        trade_dict = {}
        for trade in trades:
            date, code, name, direction, price, qty, amount, reason, time, detail = trade
        ...
        # 检查同一股票同日买卖
        for code, td in trade_dict.items():
            if td['buy'] and td['sell']:
                buy_time = td['buy'][0]['time']
                sell_time = td['sell'][0]['time']
                
                report += f"⚠️ **{code} 同日双向交易**\n"
                report += f"  - 买入: {buy_time}\n"
                report += f"  - 卖出: {sell_time}\n"
        ...
        for trade in trades:
            ...
            report += f"  - 价格: ¥{price:.2f} | 数量: {qty}股 | 金额: ¥{amount:,.2f}\n"
            report += f"  - 信号: {reason}\n\n"
...
def add_tasks_to_pm_db(issues):
    ...
    for issue in issues:
        task_id = f"TASK-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{issue['type']}"
...
def push_report_to_wecom(report):
    ...
    try:
        ...
        result = subprocess.run([
            ...
            'push_markdown(open("temp_report.md", "r", encoding="utf-8").read())'
        ], 
        capture_output=True, 
        text=True,
