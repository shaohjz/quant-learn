...
def check_pm_status():
    ...
    print(f'  📈 总计: {req_total} (done: {req_done}, testing: {req_testing}, in_progress: {req_in_progress}, pending: {req_pending})')
    ...
    print(f'  📈 总计: {bug_total} (open: {bug_open}, fixed: {bug_fixed}, verified: {bug_verified}, closed: {bug_closed})')
    
    # 3. 检查 testing 需求长时间未扭转
    print('\n=== ⏰ 长时间未更新的任务检查 ===')
...
if __name__ == '__main__':
    stats = check_pm_status()
    print('\n=== 📊 统计摘要 ===')
    print(f'需求: {stats["req_done"]}/{stats["req_total"]} 完成, {stats["req_testing"]} 个测试中, {stats["req_pending"]} 个待处理')
    print(f'Bug: {stats["bug_open"]} 个开放, {stats["bug_fixed"]} 个已修复, {stats["bug_verified"]} 个已验证')