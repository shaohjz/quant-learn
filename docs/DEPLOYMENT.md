...
> **更新**:2026-09-14(origin 切回工蜂并合并 GitHub 独有改动;3-windows 跑时钟;扫描 schtasks 默认停);2026-09-16(TES-23 修复 VqlearnLive rc=3:bat 被回迁 git 写回 LF-only + 多行 if 括号块致 cmd 解析错乱--bat 已转 CRLF+单行 if,`.gitattributes` 固定 `eol=crlf`,venv 补装 `vnpy_paperaccount`)
...
| 哪套 | 命令/界面 | 干什么 | 数量 |
|------|-----------|--------|------|
| **Windows ProdClock schtasks** | `schtasks \QuantLearn_ProdClock`（工作日 08:25 起每 10 分重复至 21:00，S4U 后台） | **全部交易脚本**：`prod_clock` 每 10 分钟自己判断。**2026-09-16 起为唯一时钟**（零 LLM，不吃平台配额） | **1 条时钟** |
| **OpenClaw LLM** | `openclaw cron` · `agentTurn` | 守夜 + 日报文案（PM 也可在工作台跑） | **最多 1～2 条** |
| **其余 Windows 任务计划** | `schtasks` | 扫描类默认停；VqlearnLive / SignalLedger / StrategyScorecard / FinanceManager / IntradayScanner / WeeklyStrategyReview 为回迁探针单独启用 | 见 CRON_JOBS |
...
**时钟 = Windows schtasks `\QuantLearn_ProdClock`（工作日 08:25 起每 10 分钟重复至 21:00，S4U 后台运行，不依赖登录会话）** → `scripts\prod_clock_runner.bat` → `prod_clock.py`。  
**2026-09-16 迁回 schtasks**：9-14 建的 OpenClaw `quant-prod-clock` cron 实际是 agentTurn（每 tick 一次 LLM 调用，一天 100+ 次），9-15 16:20 起打爆平台「定时任务 50 次/天」配额，连续 9 次 rate_limit 后停摆，守夜/日报也被连坐禁用。该 cron 已加废弃说明**永久禁用，勿再启用**；`prod_clock_runner.bat` 头部本就注明「禁止做成 LLM agentTurn」。扫描类任务在窗口内由时钟承担。交易日判定用 `sim/trade_calendar.py`(akshare 官方日历 + `data/trade_dates.json` 7 天 TTL;拉不到且无缓存时回退周末判定),节假日时钟空转不产出。
...
- 双开检查：迁入时钟的扫描类 schtasks 必须保持 Disabled；时钟本体只有 schtasks `\QuantLearn_ProdClock` 一条，OpenClaw `quant-prod-clock` cron 保持禁用，勿双开。
...
### 5.2 产机时钟（2026-09-16 起：schtasks 重复触发，零 LLM）

> 原「挂一条 systemEvent 时钟」做法**已废弃**：实际建成的是 agentTurn（每 tick 一次 LLM），
> 一天 100+ 次调用打爆平台「定时任务 50 次/天」配额，9-15 16:20 起停摆并连坐守夜/日报。
> 事故记录：`pm/ops/2026-09-16-clock-quota-outage.md`。

```bat
schtasks /create /f /tn "QuantLearn_ProdClock" /xml %TEMP%\prod_clock_task.xml
```

XML 模板（S4U 后台 + 工作日 08:25 起 `PT10M` 重复 `PT12H35M` 至 21:00 +
`StartWhenAvailable` + `ExecutionTimeLimit PT30M` + 电池策略关闭）见上述事故报告附录。

冒烟与验收：

```bat
.venv\Scripts\python.exe -u scripts\prod_clock.py --status
schtasks /run /tn QuantLearn_ProdClock
type output\prod_clock.log
```

- `schtasks /query /tn QuantLearn_ProdClock /v` 的 Next Run Time 应为下一个 :x5 时刻；
- `prod_clock.log` 出现当日 tick（窗口外显示「无到期任务」）；
- `openclaw cron list` 里 `quant-prod-clock` 保持 **disabled**（描述已注明废弃原因）。
...
REM 1b 08:40 动态稳定波段池(方法过滤+软上限50)
...
REM 2 QuantPulse:先建 09:35 一次触发,再打开 Windows「任务计划程序」GUI
...
REM 3 IntradayScanner:可选。同样用 Windows GUI 设重复 30 分钟到 14:30
...
REM 4b 16:08 银行股专用波段(账户 #4,独立结论)
...
REM 6b 16:35 每日策略复盘(诊断策略本身;依赖 56 已写完台账与净值)
...
REM 7 18:45 台账/PM/QA/Ops 白名单推 master(不占 LLM)
...
REM 8 20:30 再推一次 -- 承接 20:00 LLM 各类日报(同 bat,必开)
...
REM 9 REQ-100:影子信号(无 --auto-trade,只观察+shadow,不与 Pulse 双线下单)
...
REM 10 16:25 信号台账(必须在 16:20 DailyClose 之后)
...
REM 11 16:30 策略记分卡(必须在 16:25 台账之后)
...
REM 12 周五 17:00 每周策略复盘(默认只出提案,不改参数)
...
> 两处改动:1 `swing_daily_report.py` 的 `refresh_and_mark` 把有效止损改为 `max(fixed_stop, trailing_stop_price)`;2 `swing_intraday_watch.py` 的 `check_positions` 同样对齐,收盘与盘中口径一致。无 `trailing_stop_price`(为 NULL/空)时行为不变,完全兼容旧数据。
...
> **改了什么:** 费率修复后 #4 仍连续空仓(08-21 起 0 成交)。2026-09-08 用当日行情拆漏斗:16 只银行股里有人已踩均线,但被三件事挡死--1 `scan_stock` 仍读 #3 的 `swing_auto.PARAMS`(`min_net_rr=1.2` / 量比 0.8),`bank_swing_daily` 只改了日报模块的 `sdr.PARAMS`;2 `min_score_buy=5`,而 A=4、B/C/D=3,单次回踩永远买不了(当日交通银行 B/RR 2.66、建设银行 D/RR 2.80 都卡在 3<5);3 量比 0.8 对银行过严(当日 6 只量比 0.83~1.02)。
> 三处改动:1 `scan_stock` 增加 `params=`,`run_scan` 传入当前 `PARAMS`,`apply_bank_profile` 同时覆盖 `swing_auto.PARAMS`;2 `bank_swing_strategy.min_score_buy` 5→3;3 `max_volume_ratio` 0.8→0.95。
...
> 三处改动:1 `scan_stock` 新增可选 `fee_budget`,按真实建仓预算试算费率;2 `swing_params` 支持按 section 读取,银行池走 `config.yaml` 的 `bank_swing_strategy:` 段获得独立参数(`min_net_rr` 1.2→1.0、`executable_types` 增加 C/D);3 `run_scan` 的 `except Exception: pass` 改为记 WARNING 日志并带堆栈。
...
| **REQ-100 影子/刷价** | 重建 `QuantLearn_VqlearnLive`(无 auto-trade);`swing_daily` 先刷价再扫池 | **必建** schtasks 9;见上 |
...
```
【3-windows ProdClock（schtasks 每 10 分钟）；扫描 schtasks 默认停】
...
| 现象 | 处理 |
|------|------|
| **git 里没有昨天/今天台账** | **先查这张表**:1产机是否睡眠 2`TradeJournal`/`DailyClose` Last Run 3`DailyGitSync` Last Result 4`type output\daily_git_sync.log` 5立刻 `/run` 补推;OpenClaw 守夜本应已告警 |
| **git 里没有 LLM 日报(daily_reports)** | 120:00 cron 是否写了文件 `dir daily_reports` 2是否挂了 `DailyGitSyncEvening`@20:30 3提示词是否仍只推企微不落盘 → 重贴本文提示词 C 4立刻 `/run DailyGitSyncEvening` |
