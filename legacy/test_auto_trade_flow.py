...
def setup_test_signal():
    ...
    cur.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, ("600519", "贵州茅台", "test_buy", 1800.0, today, 1800.0, "triggered", 1))
    
    conn.commit()
    conn.close()
    
    print("   ✓ 测试信号已插入（600519 贵州茅台，阈值 ¥1800.0）")
...
def cleanup_test_data():
    ...
    # 删除测试信号
    cur.execute("DELETE FROM threshold_state WHERE stock_code = '600519' AND rule_name = 'test_buy'")
    
    # 删除测试交易记录
    cur.execute("DELETE FROM sim_trades WHERE stock_code = '600519' AND signal_reason LIKE '%test_buy%'")
