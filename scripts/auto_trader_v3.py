#!/usr/bin/env python3
"""
scripts/auto_trader_v3.py - 自动交易执行器(简化版)

功能:
  - 监听新生成的信号(从 threshold_state 表检测)
...
信号来源:
  threshold_state 表 - 检测 status='triggered' 的信号

使用:
  python scripts/auto_trader_v3.py              # 执行一次
  python scripts/auto_trader_v3.py --daemon    # 守护进程模式(每5分钟检查一次)
  python scripts/auto_trader_v3.py --dry-run    # 干运行(不实际执行交易)
"""
import sys
import os
import json
...
class AutoTrader:
    ...
    def execute_buy_signal(self, signal: Dict) -> bool:
        ...
        if self._has_sell_today(stock_code):
            logger.info(f"🚫 BUG-011: {stock_code} 今日已有卖出成交，拒绝再买入")
            return False
    ...
    def _has_sell_today(self, stock_code: str) -> bool:
        """BUG-011: 检查今日是否已有该股票的 SELL 成交。

        用于同日反向交易防御：若今日已卖出，则禁止再买入。
        """
        try:
            import sqlite3
            db_path = ROOT / "data" / "sim_live_mirror.db"
            ...
            cur.execute(
                ...
                (self.account_id, stock_code, today),
            )
            result = cur.fetchone() is not None
            conn.close()
            return result
        except Exception as e:
            logger.error(f"检查同日卖出记录失败 {stock_code}: {e}")
            return False
    ...
    def mark_signal_executed(self, signal: Dict):
        ...
        try:
            ...
            if signal["source"] == "threshold":
                # 更新 threshold_state 表,将 status 改为 'executed'
                cur.execute("""
