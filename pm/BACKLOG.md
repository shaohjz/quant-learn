...
> 自动生成于 2026-08-21 01:39:16（`pm_cli list --write-backlog`）。真源为各 REQ/BUG markdown。
...
| TASK-20260717-2004-001 | bug | verified | P1 | 晶方科技(603005)现价31.33已跌破移动止损31.56但持仓未卖出 |
| REQ-058 | bug | verified | P1 | 持仓清仓后 threshold_state 悬挂记录未自动失效（002709 案例） |
| REQ-043 | story | verified | P1 | 量化通知/盘中盯盘 cron 去大模型化 — agentTurn → systemEvent |
| REQ-011 | story | testing | P1 | 接入vnpy OmsEngine实现详细订单成交回放 |
| REQ-106 | story | testing | P1 | 银行股专用波段交易（账户#4 + 独立结论） |
| TASK-20260716-2004-003 | task | verified | medium | TASK-20260709-2004-001 验收退回后状态未回退，应置回 testing/open |
| TASK-20260718-2003-001 | bug | verified | medium | 清仓后 sim_positions 残留记录未清理（龙旗科技603341、山西汾酒600809 数量=0仍留存） |
...
| REQ-015 | story | verified | P2 | 支持自动获取用户手工填写的复盘要点分析 |
| REQ-027 | story | verified | P2 | 支持交易纪律打卡与知行合一评分 |
| REQ-020 | story | verified | P2 | 双账户复盘支持交易异动或异常提醒 |
| REQ-030 | story | verified | P2 | 支持自动统计策略信号触发的交易盈亏并在复盘中展示 |
| REQ-005 | story | verified | P2 | REQ-005: 观察列表K线缩略图 |
| REQ-019 | story | testing | P2 | 支持实时同步并展示QMT订单/成交状态流 |
