...
- [x] **波段策略回测（研究版）**：A/B 已接共享现金、T+1、Walk-forward 与 2×成本；旧 CSV 缺 raw 元数据，暂不晋级（见 `STRATEGY_RESEARCH_2026-07-31.md`）
...
### 定时任务（待添加）
- [x] **波段扫描结果推送**：`swing_auto.py` 由 `swing_pool_builder_runner.bat`（08:40）在建池后调用并推企微
