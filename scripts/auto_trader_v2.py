#!/usr/bin/env python3
"""
scripts/auto_trader_v2.py — 自动交易执行器（完善版）
...
功能：
  ...
  - 支持买入和卖出信号
  - 仓位管理（单只仓位上限15%）
  - 止盈策略（浮盈≥30%卖出50%，≥50%卖出剩余）
  - 记录执行日志

信号来源：
  1. threshold_state 表 - 阈值触发信号
  2. sim_trades 表 - 新生成的交易信号（尚未执行）
  3. strategy_shadow_signals 表 - 策略影子信号

使用：
  python scripts/auto_trader_v2.py              # 执行一次
  python scripts/auto_trader_v2.py --daemon    # 守护进程模式（每5分钟检查一次）
  python scripts/auto_trader_v2.py --dry-run    # 干运行（不实际执行交易）
"""
import sys
import os
import json
...
class AutoTrader:
    ...
    def check_new_signals(self) -> List[Dict]:
        ...
        try:
            ...
            # 1. 从 threshold_state 表检测新触发的阈值信号
            # 条件：status='triggered' 且今天首次触发
            ...
            for row in rows:
                ...
                logger.info(f"发现新阈值信号: {signal['stock_code']} ({signal['stock_name']}) - {signal['rule_name']}")
            
            # 2. 从 strategy_shadow_signals 表检测新的策略信号
            cur.execute("""
                SELECT id, shadow_date, shadow_time, strategy_id, stock_code,
                       stock_name, price, position, signal_action, signal_rule,
                       signal_reason, confidence
                FROM strategy_shadow_signals
                WHERE executed = 0
                ORDER BY shadow_date, shadow_time
            """)
            ...
            for row in rows:
                signal = {
                    ...
                    "trigger_date": row["shadow_date"],
                    "trigger_time": row["shadow_time"],
                    "trigger_price": row["price"],
                    "confidence": row["confidence"],
                    "signal_type": row["signal_action"],  # 'buy' or 'sell'
                }
                new_signals.append(signal)
                logger.info(f"发现新策略信号: {signal['stock_code']} ({signal['stock_name']}) - {signal['strategy_id']} {signal['signal_action']}")
    ...
    def execute_buy_signal(self, signal: Dict) -> bool:
        ...
        # 构建信号原因
        signal_reason = f"auto_trade|{signal.get('rule_name', signal.get('strategy_id', 'unknown'))}|触发价{signal.get('trigger_price', 0):.3f}"
    ...
    def mark_signal_executed(self, signal: Dict):
        ...
        try:
            ...
            if signal["source"] == "threshold":
                # 更新 threshold_state 表
                cur.execute("""
            ...
            elif signal["source"] == "strategy_shadow":
                # 更新 strategy_shadow_signals 表
                cur.execute("""
            ...
            logger.info(f"标记信号为已执行: {signal['source']} ID {signal['id']}")
        except Exception as e:
