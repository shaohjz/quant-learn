...
## #3 波段仓（account_id=3） `spec_hash=ef4507ea4d39`
...
| 参数 | 当前值 | 取自 | 说明 |
|------|-------|------|------|
| 可成交信号类型 | **G** | `swing_params:execution.executable_types`<br>`scripts/swing_daily_report.py:EXECUTABLE_TYPES`<br>`scripts/swing_intraday_watch.py:EXECUTABLE` | A=缩量回踩MA20 B=缩量回踩MA10；C/D/E/F 只观察 |
...
| 参数 | 当前值 | 取自 | 说明 |
|------|-------|------|------|
| 止损 | **12%** | `swing_params:execution.stop_loss_pct`<br>`scripts/swing_daily_report.py:STOP_LOSS_PCT`<br>`scripts/swing_intraday_watch.py:STOP_LOSS_PCT` |  |
| 止盈 | **20%** | `swing_params:execution.take_profit_pct`<br>`scripts/swing_daily_report.py:TAKE_PROFIT_PCT`<br>`scripts/swing_intraday_watch.py:TAKE_PROFIT_PCT` |  |

### 仓位与资金

| 参数 | 当前值 | 取自 | 说明 |
|------|-------|------|------|
| 最大持仓数 | **3只** | `swing_params:execution.max_positions`<br>`scripts/swing_daily_report.py:MAX_POSITIONS` |  |
| 单笔预算 | **1.5 万** | `swing_params:execution.single_budget`<br>`scripts/swing_daily_report.py:SINGLE_BUDGET` |  |
...
## #1 学习仓（account_id=1） `spec_hash=7d22651aaa1b`
...
| 参数 | 当前值 | 取自 | 说明 |
|------|-------|------|------|
| 硬止损 | **-12%** | `config.yaml:risk.stop_loss_pct` |  |
| 止盈线 | **20%** | `config.yaml:risk.take_profit_pct` |  |
| 止盈方式 | **full** | `config.yaml:risk.take_profit_mode` | half=首次止盈卖一半 |
| 跟踪止损 | **atr_hybrid** | `config.yaml:risk.trailing.mode` |  |
| 跟踪启动浮盈 | **15%** | `config.yaml:risk.trailing.activate_profit_pct` |  |

### 仓位与资金

| 参数 | 当前值 | 取自 | 说明 |
|------|-------|------|------|
| 单笔预算 ❌未取到 | **（未取到）** | `scripts/sim_executor.py:DEFAULT_BUY_BUDGET` |  |
| 最大持仓数 | **5只** | `config.yaml:risk.max_total_positions` |  |
