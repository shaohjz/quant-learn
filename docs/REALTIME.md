...
盘前 ~08:40     SwingPool 方法过滤池≤50 + swing_auto 盘前机会推企微
盘中 每10分     QuantPulse = 真仓阈值 + 波段机会(扫方法池) + 指数（主心跳）；模拟成交立刻 @all
...
| `swing_pool_builder.py` | 沪深300+中证500 → 方法过滤+软上限50 | 盘前 08:40 | 建池；同 bat 调 `swing_auto` 推企微 |
