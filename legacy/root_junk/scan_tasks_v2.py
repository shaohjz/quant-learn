#!/usr/bin/env python3
"""
正确扫描 pm/requirements/ 和 pm/bugs/ 中的任务当前状态
"""
import os
import re
from pathlib import Path

def extract_current_status(file_path):
    """从文件中提取当前状态（在基本信息表格中）"""
    try:
...
def extract_priority(file_path):
    ...
    try:
        ...
        # 查找基本信息表格中的优先级行
        priority_match = re.search(r'\|\s*\*\*优先级\*\*\s*\|\s*(\S+)\s*\|', content)
        if priority_match:
...
def scan_directory(directory, task_type):
    ...
    try:
        for file_path in Path(directory).glob('*.md'):
            status = extract_current_status(file_path)
            priority = extract_priority(file_path)
            
            if status and status.lower() in [s.lower() for s in pending_statuses]:
                tasks.append({
                    'file': file_path.name,
                    'path': str(file_path),
                    'status': status,
